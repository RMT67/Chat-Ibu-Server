...
const validateInput = (data, schema) => {
  for (const [key, validator] of Object.entries(schema)) {
    ...
    if (data[key] !== undefined && validator.type && typeof data[key] !== validator.type) {
      return { valid: false, error: `${key} must be ${validator.type}` };
    }
    if (data[key] !== undefined && validator.validate && !validator.validate(data[key])) {
    ...
    socket.on('join:room', async (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'JOIN')) {
          socket.emit('error', { message: 'Too many join requests. Please wait.' });
          return;
        }
    ...
    socket.on('disconnect', async () => {
      ...
      if (authenticatedUserId) {
        try {
          ...
          io.emit('user:offline', { userId: authenticatedUserId, isOnline: false });
        } catch (error) {
