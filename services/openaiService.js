...
Pastikan konten menarik, relevan untuk komunitas ibu-ibu, dan memicu diskusi yang bermakna.`;
  ...
  try {
    ...
    return {
      ...
      description:
        parsed?.description ||
        "Tempat diskusi yang nyaman untuk berbagi pengalaman dan tips",
...
const generateOpeningMessage = async (
  ...
  if (userId && !checkRateLimit(userId, "GENERATE_MESSAGE")) {
    ...
    return `Selamat datang di ${sanitizeInput(
      roomName
    )}! Mari kita diskusikan tentang ${sanitizeInput(
      roomTopic
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant that generates opening messages for Indonesian mothers community chat rooms. Always respond with only the message text, no formatting, no prefixes.",
        },
        {
          role: "user",
          content: prompt,
